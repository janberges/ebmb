...
\begin{document}
   ...
   \begin{enumerate}
      \item
         ...
         real axis via \textsc{Pad\'e} approximants. The normal-state equations
   ...
   The makefile is designed for the \emph{GNU}, \emph{Intel}, and \emph{LLVM}
   Fortran compilers:
   %
   \begin{quote}
   ...
   \end{quote}
   %
   \emph{LAPACK} and \emph{BLAS} are required. To also install the \emph{Python}
   wrapper:
   %
   \begin{quote}
   ...
   Copyright \copyright~2016--2026 Jan Berges
   ...
   \begin{equation*}
      ...
      _{\displaystyle (\delta \omega_n)_i} \mat 1
      + \underbrace{\Delta_i(n) \, Z_i(n)}
      _{\displaystyle \phi_i(n)} \mat \sigma_1
      + \chi_i(n) \mat \sigma_3,
   \end{equation*}
   %
   ...
   \footnote{%
      ...
      \textsc{Mitrovi\'c} in Solid state physics \textbf{37} (1982)
      }
   %
   \begin{equation}
      \begin{split}
   ...
   \rightarrow \omega + \I \eta$) by means of \textsc{Pad\'e} approximants.%
   %
   \footnote{%
   ...
   \begin{align}
      ...
      \end{cases}
      &
      \mat \mu^*(\omega_{N \sub C}) &= \Bigl(
          \mat 1 + \mat \mu^* \ln \frac{\omega \sub E}{\omega_{N \sub C}}
      \Bigr)^{-1} \cdot \mat \mu^*
   ...
   %
   \begin{align}
       \mat \mu^*(\omega_{N \sub C}) &= \mat S^{-1} \cdot \mat \mu,
       &
       S_{i j} &= \delta_{i j} +
       \frac {\mu_{i j}} \pi \int \from{-\infty} \till \infty \D \epsilon
       \frac{n_j(\epsilon)}{n_j(\mu_0)}
       \begin{cases}
           \frac 1 {\epsilon - \mu_0}
   ...
   %
   which allows for exact analytic continuation without Pad\'e approximants.
   They are implemented for the normal state:
   %
   \begin{equation}
   ...
   \begin{itemize}
      ...
      \item
         ...
         overwritten. For all keys, a value of \texttt{None} corresponds to the
         default.
   \end{itemize}
   ...
   \begin{table}[b]
      ...
      \begin{tabular}{*9l}
         ...
         [\gap]
         \tt gap           & \tt false   & --               & --                  & find measurable gap?                       & $+$ & $-$ & $-$ & $-$ \\
         [\gap]
         \tt unscale       & \tt true    & --               & --                  & estimate missing {\tt muC} from {\tt mu*}? & $+$ & $+$ & $+$ & $-$ \\
         \tt rescale       & \tt true    & --               & --                  & use $\mu^*_{i j}$ rescaled for cutoff?     & $+$ & $+$ & $+$ & $-$ \\
         \tt cdosfix       & \tt true    & --               & --   & use $D_i^\infty(n)$ in Eq.~\ref{linearized gap equation}? & $-$ & $-$ & $+$ & $-$ \\
         [\gap]
         \tt apade         & \tt true    & --               & --                  & get $A_i(\omega)$ from $G_i(\I \omega_n)$? & $+$ & $-$ & $-$ & $-$ \\
         \tt power         & \tt true    & --               & --                  & power method for single band?              & $-$ & $-$ & $+$ & $-$ \\
         [\gap]
         \tt realgw        & \tt false   & --               & --                  & do real-axis GW$_0$ calculation?           & $+$ & $-$ & $-$ & $-$ \\
         \tt krakro        & \tt true    & --    & -- & send $\eta \rightarrow 0^+$ in $\Im \Sigma_{1 1 i}(\omega + \I \eta)$? & $+$ & $-$ & $-$ & $-$
      \end{tabular}
      \caption{Input parameters. The keys are case insensitive.}
      \label{parameters}
   \end{table}
   ...
   \begin{table}
      ...
      \begin{tabular}{*2l r}
         ...
         \tt domega & frequency shift                         & $(\delta \omega_n)_i$ \\
         \tt Z      & renormalization                             & $Z_i(n)$          \\
         \tt Delta  & gap                                         & $\Delta_i(n)$     \\
         \tt phi    & order parameter                             & $\phi_i(n)$       \\
         ...
         \tt status & steps until convergence or $-1$             & --                \\
         \hline
         ...
         \tt \stack{l}{n0} {n}  & \stack{r}{initial}{final} $\Big\}$ particle number    & \stack{r}{$n_0$}  {$n$}   \\
         ...
         \tt \stack{l}{Re[phi]}  {Im[phi]}   & \stack{r}{real}{imaginary} $\Big\}$ order parameter & $\phi_i(\omega)$   \\
         ...
         \hline
         \multicolumn3l{\textbf{gap results} \hfill \texttt{gap=true}} \\
         \hline
         \tt Delta0  & measurable gap                  & \llap{${\Delta_0}_i = \operatorname{Re}[\Delta_i({\Delta_0}_i)]$} \\
         \tt status0 & steps until convergence or $-1$ & --
      \end{tabular}
